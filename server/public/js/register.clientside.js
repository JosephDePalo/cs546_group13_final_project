(function ($) {
    ...
    signup_form.addEventListener("submit", (event) => {
        ...
        if (email.value.length === 0) {
            let error_message = document.createElement("dd");
            error_message.append("Error: email cannot be an empty string or string of spaces!");
            errors.appendChild(error_message);
            $(errors).show();
            errors_count++;
        }
        ...
        if (!email_address_regex.test(email.value)) {
            let error_message = document.createElement("dd");
            error_message.append("Error: email_address must be in email address format!");
            errors.appendChild(error_message);
            $(errors).show();
            errors_count++;
        }
        ...
        if (confirm_password.value !== password.value) {
            let error_message = document.createElement("dd");
            error_message.append("Error: confirm_password must be the same as password!");
            errors.appendChild(error_message);
            $(errors).show();
            errors_count++;
        }
